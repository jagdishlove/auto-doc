...
export const DocViewer: React.FC<DocViewerProps> = ({ projectDocs, selectedDoc, loading, isBatchScanning, onViewDoc }) => {
  if (loading) {
    return (
  ...
  if (isBatchScanning && projectDocs.length === 0) {
     return (
        <div className="h-full flex flex-col items-center justify-center text-gray-500 border-2 border-dashed border-white/10 rounded-xl bg-white/5">
          ...
          <p className="text-xs text-gray-600 mt-2">Checking heuristics and finding endpoints</p>
        </div>
     );
  }
  ...
  if (projectDocs.length === 0 && !selectedDoc) {
    return (
      <div className="h-full flex flex-col items-center justify-center text-gray-500 border-2 border-dashed border-white/10 rounded-xl bg-white/5">
        ...
        <p className="text-sm mt-2">Select a file or click <span className="text-brand-400 font-bold">Auto-Scan</span> to begin.</p>
      </div>
    );
  }
        ...
        <div className="bg-dark-800 rounded-xl border border-white/10 p-8 shadow-xl">
          <div className="border-b border-white/10 pb-6 mb-6">
            <div className="flex items-center gap-3 mb-2">
              ...
              <button onClick={() => onViewDoc('')} className="text-gray-500 text-xs hover:text-white transition-colors hover:underline">
                 ← Back to Project Overview
              </button>
            </div>
            <h1 className="text-3xl font-bold text-white mb-2">{selectedDoc.fileName}</h1>
            <p className="text-gray-400 leading-relaxed">{selectedDoc.overview}</p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4 mb-8">
            <div className="bg-white/5 rounded-lg p-3 border border-white/5 flex items-center gap-3">
              ...
              <div>
                <div className="text-lg font-bold text-white">{selectedDoc.astMetrics.functionsFound}</div>
            ...
            <div className="bg-white/5 rounded-lg p-3 border border-white/5 flex items-center gap-3">
              ...
              <div>
                <div className="text-lg font-bold text-white">{selectedDoc.astMetrics.classesFound}</div>
            ...
            <div className="bg-white/5 rounded-lg p-3 border border-white/5 flex items-center gap-3">
              ...
              <div>
                <div className="text-lg font-bold text-white">{selectedDoc.astMetrics.importsFound}</div>
          ...
          <div className="space-y-4">
            ...
            {selectedDoc.endpoints.length === 0 && (
              <p className="text-gray-500 italic">No public endpoints detected in this file.</p>
            )}
       ...
       <div className="bg-dark-800 rounded-xl border border-white/10 p-8 shadow-xl min-h-full">
          <div className="border-b border-white/10 pb-6 mb-6">
             ...
             <p className="text-gray-400">
                Consolidated API documentation from {projectDocs.length} analyzed file{projectDocs.length !== 1 && 's'}.
             </p>
          </div>
          ...
          {allEndpoints.length === 0 ? (
             <div className="p-8 text-center text-gray-500 bg-white/5 rounded-lg">
                ...
                <p className="text-sm mt-2">Try manually selecting a different file.</p>
             </div>
          ) : (
             <div className="space-y-4">
